export const translations = {
  fr: {
    "nav.founders": "Pour les fondateurs",
    "nav.home": "Accueil",
    "nav.about": "À propos",
    "nav.useCases": "Cas d'usage",
    "nav.communityBuilders": "Pour les community builders",
    "nav.organisations": "Pour les organisations",
    "nav.cta.apply": "Postuler",
    "nav.cta.beta": "Demander une communauté",
    "nav.cta.partner": "Devenir partenaire",
    "nav.cta.bookCall": "Réserver un appel",
    "nav.cta.discovery": "Réserver un appel découverte",

    "home.hero.eyebrow": "APPRENTISSAGE ENTRE PAIRS POUR CEUX QUI REFUSENT DE DÉCROCHER",
    "home.hero.title.lead": "Soyez curieux. Ayez un but. Restez en avance.",
    "home.hero.title.accent": "Ensemble.",
    "home.hero.subtitle": "SOULCHAIN associe un insight quotidien calibré sur votre situation à un réseau de pairs choisis selon votre façon de penser. Construit autour de ce que l'IA ne peut pas vous donner : connexion humaine, expérience vécue, et les personnes qui sont passées par là où vous allez.",
    "home.hero.cta.primary": "Postuler pour rejoindre",
    "home.hero.cta.secondary": "Voir comment ça marche",

    "home.doors.eyebrow": "TROIS PORTES · UNE PLATEFORME",
    "home.doors.title": "Un produit. Trois portes d'entrée.",
    "home.doors.subtitle": "Nous avons construit SOULCHAIN pour trois audiences. Le produit est un. Les portes sont trois.",
    "home.doors.founders.eyebrow": "POUR LES FONDATEURS",
    "home.doors.founders.title": "Pensé pour les fondateurs, en premier.",
    "home.doors.founders.body": "Une habitude quotidienne et une cohorte d'opérateurs qui apprennent à vos côtés. Pour des fondateurs qui dirigent de vraies entreprises et refusent d'apprendre seuls.",
    "home.doors.founders.link": "En savoir plus →",
    "home.doors.builders.eyebrow": "POUR LES COMMUNITY BUILDERS",
    "home.doors.builders.title": "Pour les audiences qui paient pour apprendre de vous.",
    "home.doors.builders.body": "La couche de rétention pour les fondateurs, figures publiques et créateurs éducatifs. Vous apportez la marque et l'audience. Nous apportons le système qui les fait rester.",
    "home.doors.builders.link": "En savoir plus →",
    "home.doors.enterprise.eyebrow": "POUR LES ORGANISATIONS",
    "home.doors.enterprise.title": "Pour les équipes derrière le travail.",
    "home.doors.enterprise.body": "Une infrastructure cognitive pour les personnes que vous choisissez d'inclure. Des dirigeants aux opérationnels. D'un département à toute l'entreprise.",
    "home.doors.enterprise.link": "En savoir plus →",

    "home.pillars.eyebrow": "CE QUE SOULCHAIN EST VRAIMENT",
    "home.pillars.title": "Construit autour de ce que l'IA ne peut pas remplacer.",
    "home.pillars.subtitle": "SOULCHAIN n'est pas une application d'apprentissage. C'est une plateforme d'apprentissage entre pairs organisée autour de quatre piliers.",
    "home.pillars.p1.title": "Habitude quotidienne",
    "home.pillars.p1.body": "Cinq minutes par jour. Un insight personnalisé calibré sur où vous en êtes, dans le format qui convient à votre moment : texte, audio ou podcast. Le rythme que les abonnements IA ne structurent pas.",
    "home.pillars.p1.bodyLine1": "Cinq minutes par jour. Un insight personnalisé calibré sur où vous en êtes,",
    "home.pillars.p1.bodyLine2": "dans le format qui convient à votre moment : texte, audio ou podcast.",
    "home.pillars.p1.bodyLine3": "Le rythme que les abonnements IA ne structurent pas.",
    "home.pillars.p2.title": "Matching cognitif",
    "home.pillars.p2.body": "Un algorithme qui vous associe à des personnes qui pensent comme vous. Plus vous utilisez SOULCHAIN, plus les correspondances s'affinent. C'est la partie que les concurrents ne peuvent pas reproduire en ajoutant des fonctionnalités.",
    "home.pillars.p3.title": "Présence de pairs",
    "home.pillars.p3.body": "De vraies personnes dans de vraies conversations. Des opérateurs qui ont déjà pris la décision que vous affrontez cette semaine. Ce que ChatGPT, Claude et Perplexity ne peuvent fondamentalement pas remplacer, quelle que soit la qualité du modèle.",
    "home.pillars.p4.title": "Expérience vécue",
    "home.pillars.p4.body": "Une communauté est plus qu'une liste de contacts. SOULCHAIN vous donne la structure pour transformer un insight en conversation, une conversation en relation, et une relation en pensée que vous ne pouvez pas avoir seul.",

    "home.whynow.eyebrow": "Pourquoi maintenant",
    "home.whynow.title": "Trois conditions viennent d'ouvrir la fenêtre.",
    "home.whynow.col1.stat": "20$",
    "home.whynow.col1.caption": "Par mois",
    "home.whynow.col1.title": "L'IA absorbe le substituable.",
    "home.whynow.col1.body": "Recherche, synthèse, distribution de contenu — banalisés par l'IA générative à 20$ par mois. Ce qui reste précieux, c'est ce que l'IA ne peut pas remplacer : la présence de pairs, le matching cognitif, l'habitude.",
    "home.whynow.col2.stat": "55%",
    "home.whynow.col2.caption": "Des PDG · 2024",
    "home.whynow.col2.title": "L'isolement est désormais nommé publiquement.",
    "home.whynow.col2.body": "55% des CEOs ont déclaré des troubles de santé mentale en 2024, en hausse de 24 points sur 2023. Prises de parole publiques chez Airbnb, Apple, PepsiCo. Le coût de l'isolement devient mesurable.",
    "home.whynow.col3.stat": "N°1",
    "home.whynow.col3.caption": "Raison citée",
    "home.whynow.col3.title": "Le décalage de niveau est la critique universelle.",
    "home.whynow.col3.body": "La première raison pour laquelle les gens quittent les groupes de pairs traditionnels est d'être groupés avec des pairs qui ne correspondent pas à leur stade. Les modèles existants ne peuvent pas régler ça à grande échelle. Le matching cognitif, si.",

    "home.proof.eyebrow": "Pourquoi l'habitude quotidienne + la présence de pairs fonctionnent",
    "home.proof.headlineLead": "La plupart des apprentissages sont oubliés avant de s'ancrer. ",
    "home.proof.headlineEm": "Ceci n'est pas un apprentissage.",
    "home.proof.stat1.range": "64–90",
    "home.proof.stat1.label": "Taux d'achèvement avec apprentissage communautaire et en cohorte",
    "home.proof.stat2.range": "3–15",
    "home.proof.stat2.label": "Taux d'achèvement avec l'apprentissage classique en autonomie",
    "home.proof.citation": "Référence sectorielle — recherche sur l'apprentissage en cohorte, 2024.",
    "home.proof.bodyLead": "SOULCHAIN est construit sur le stack de rétention : ",
    "home.proof.bodyStrong": "micro-stimulation quotidienne, redevabilité entre pairs, conversation appliquée.",
    "home.proof.bodyTail": " Ce que vous engagez le premier jour se retrouve dans vos décisions au quatre-vingt-dixième.",

    "home.quote.text": "Soyez curieux. Ayez un but. Donnez du sens à votre vie. Cultivez ce sens. Aimez cette vie.",
    "home.quote.attribution": "KARIM AMOR · FONDATEUR, EPINEON",

    "home.about.eyebrow": "À PROPOS D'EPINEON",
    "home.about.title": "Une verticale d'Epineon.",
    "home.about.body": "SOULCHAIN est construit par Epineon, une entreprise qui travaille sur ce que nous pensons que l'IA ne peut pas remplacer : matching cognitif, habitude quotidienne, présence de pairs, identité. SOULCHAIN est la première application de cette thèse. D'autres suivront.",
    "home.about.link": "Visiter Epineon →",

    "home.cta.title": "Créez votre communauté SOULCHAIN.",
    "home.cta.subtitle": "Soumettez une demande. Nous l'examinons manuellement et vous répondons rapidement.",
    "home.cta.primary": "Demander une communauté",
    "home.cta.builders": "Vous construisez une communauté ? Parlez-nous →",
    "home.cta.enterprise": "Vous l'apportez à votre équipe ? Réserver un appel →",

    "footer.tagline": "Une verticale d'Epineon - l'ingénierie de la facon dont les gens pensent.",
    "footer.product.founders": "Pour les fondateurs",
    "footer.product.communityBuilders": "Pour les community builders",
    "footer.product.organisations": "Pour les organisations",
    "footer.downloadApp.title": "Telechargez notre application",
    "footer.company.about": "A propos d'Epineon",
    "footer.company.press": "Presse",
    "footer.company.careers": "Carrieres",
    "footer.legal.terms": "Conditions",
    "footer.legal.privacy": "Confidentialite",
    "footer.legal.cookies": "Cookies",
    "footer.legal.contact": "Contact",
    "footer.copyright": "© 2026 Epineon - SOULCHAIN. Casablanca - Concu pour les decideurs du monde entier.",

    "hero.eyebrow": "POUR LES FONDATEURS",
    "hero.title.lead": "L'IA vous donne des réponses.",
    "hero.title.accent": "Elle ne vous donne pas de conviction.",
    "hero.subtitle": "Cinq minutes d'insights quotidiens calibrés sur où vous en êtes. Des conversations quotidiennes entre opérateurs de votre niveau. Un matching cognitif IA qui reconstruit le cercle que vous ne pensiez plus pouvoir reformer.",
    "hero.cta.primary": "Postuler pour rejoindre",
    "hero.cta.secondary": "Comment ça marche",
    "hero.brandLine": "Une verticale d'Epineon",

    "trust.label": "Fondateurs entre 1M$ et 50M$ de revenus. France · USA · Moyen-Orient · Asie.",
    "trust.stat1": "4 continents.",
    "trust.stat2": "Opérateurs uniquement.",

    "pain.eyebrow": "LÀ OÙ ÇA DEVIENT DIFFICILE",
    "pain.title": "À votre niveau, apprendre n'est pas un problème de contenu. C'est un problème de pairs.",
    "pain.body": "Vous lisez sans arrêt. Vous écoutez des podcasts en accéléré. Vous posez trente questions par jour à ChatGPT. Et le manque est toujours là. Vous n'avez plus assez de personnes qui opèrent à votre échelle, qui challengent honnêtement votre pensée, qui étaient dans la pièce quand la décision que vous prenez cette semaine a déjà été tranchée. Plus vous grandissez, plus ce cercle rétrécit.",
    "pain.imageAlt": "Dirigeant seul la nuit dans une salle de conseil surplombant la ville",

    "bring.eyebrow": "CE QUE NOUS APPORTONS",
    "bring.title": "Un système d'apprentissage construit autour de ce que l'IA ne peut pas remplacer.",
    "bring.card1.title": "Brain Booster personnalisé",
    "bring.card1.body": "Cinq minutes par jour, dans le format qui convient à votre moment. Texte, audio ou podcast. Calibré sur votre secteur, votre stade, vos thèmes. Les thèmes de départ (Pro, Santé, Sport, Famille, Hobbies) sont des suggestions ; vous ajoutez les vôtres. Chaque booster se termine par un quiz rapide qui vérifie que l'insight s'est vraiment ancré.",
    "bring.card2.title": "Near By",
    "bring.card2.body": "Où que vous soyez, SOULCHAIN fait remonter les membres de votre communauté qui sont à proximité — des personnes avec qui vous avez déjà connecté, désormais à portée. Envoyez un message. Organisez un café. La plateforme transforme le matching entre pairs en présence dans le monde réel.",
    "bring.card3.title": "Daily Groups",
    "bring.card3.body": "Chaque jour, SOULCHAIN vous groupe avec d'autres membres dont le booster recoupe le vôtre. Un sujet. Une conversation. Quand la journée se termine, la discussion passe. Pas de chat mort. Pas de feed infini.",
    "bring.card4.title": "Peers",
    "bring.card4.body": "Une liste de personnes dans votre communauté qui pensent comme vous. Connectez-vous directement avec n'importe qui sur la liste. Le type de relations un-à-un pour lesquelles on prendrait l'avion — identifiées par un algorithme entraîné sur l'affinité cognitive.",

    "how.eyebrow": "COMMENT ÇA MARCHE",
    "how.title": "Quatre étapes. Puis ça tourne.",
    "how.step1.title": "Rejoindre la beta",
    "how.step1.body": "Un formulaire de deux minutes. Nous ajoutons de nouveaux membres au fur et à mesure. Une fois entré, vous créez votre profil et choisissez vos thèmes de départ.",
    "how.step2.title": "Être mis en relation",
    "how.step2.body": "Le matching cognitif vous associe à des conversations quotidiennes et à des connexions un-à-un selon votre façon de penser, votre stade, votre secteur.",
    "how.step3.title": "Pratiquer chaque jour",
    "how.step3.body": "Cinq minutes de Brain Booster le matin. Une conversation entre pairs quand vous avez dix minutes. Une liste de pairs compatibles quand vous avez une question qui demande une réponse humaine.",
    "how.step4.title": "Cumuler",
    "how.step4.body": "Habitude quotidienne, redevabilité entre pairs, conversation appliquée. Trois mois plus tard, l'affûtage se voit dans les décisions que vous prenez sous pression.",
    "how.outcome": "Mieux décider. Réseau plus fort. Moins d'isolement.",

    "whyFounders.eyebrow": "POURQUOI LES FONDATEURS D'ABORD",
    "whyFounders.title": "Un focus, pas un filtre.",
    "whyFounders.highlight": "les vingt ans d'expérience d'opérateur de Karim",
    "whyFounders.body": "SOULCHAIN est construit pour quiconque refuse d'apprendre seul. Mais nous avons construit la première version pour les fondateurs parce que c'est là où notre réseau est le plus aiguisé — là où les vingt ans d'expérience d'opérateur de Karim et les personnes de son cercle s'alignent. Le prisme fondateur façonne le discours, le ton et la cohorte initiale. Le produit lui-même fonctionne pour quiconque le rejoint.\n\nSi vous êtes fondateur, la porte est directe. Si vous ne l'êtes pas, la porte est la même — les conversations apportent simplement un angle différent.",

    "whynow.eyebrow": "POURQUOI MAINTENANT",
    "whynow.title": "Trois conditions viennent d'ouvrir la fenêtre.",
    "whynow.col1.stat": "20$",
    "whynow.col1.title": "L'IA absorbe le substituable.",
    "whynow.col1.body": "Recherche, synthèse, distribution de contenu — banalisés par l'IA générative à 20$ par mois. Ce qui reste précieux, c'est ce que l'IA ne peut pas remplacer : la présence de pairs, le matching cognitif, le rituel.",
    "whynow.col2.stat": "55%",
    "whynow.col2.title": "L'isolement est désormais nommé publiquement.",
    "whynow.col2.body": "55% des CEOs ont déclaré des troubles de santé mentale en 2024 — en hausse de 24 points sur 2023. Prises de parole publiques chez Airbnb, Apple, PepsiCo. Le coût de l'isolement devient mesurable.",
    "whynow.col3.stat": "N°1",
    "whynow.col3.title": "Le décalage de niveau est la critique universelle.",
    "whynow.col3.body": "La première raison pour laquelle les CEOs quittent Vistage, YPO et EO est d'être groupés avec des pairs qui ne correspondent pas à leur stade. Les modèles existants ne peuvent pas régler ça à grande échelle. Le matching cognitif, si.",

    "cohort.eyebrow": "LA COHORTE DES FONDATEURS",
    "cohort.title": "Choisi, pas démarché.",
    "cohort.body": "SOULCHAIN est un réseau de pairs avec une porte. Nous examinons chaque candidature. Nous cherchons des fondateurs et CEOs qui dirigent des entreprises entre 1M$ et 50M$ de revenus, qui ont dépassé leur premier réseau, et qui sont prêts à honorer le rituel quotidien. La sélection est ce qui rend la pièce digne d'être occupée. La cohorte est la raison d'être de la plateforme.",
    "cohort.cta": "Postuler à la cohorte",

    "bridge.eyebrow": "VOUS N'ÊTES PAS FONDATEUR ?",
    "bridge.card1.label": "Pour les community builders",
    "bridge.card1.title": "Apportez SOULCHAIN à votre audience.",
    "bridge.card1.body": "Fondateurs, figures publiques et créateurs éducatifs utilisent SOULCHAIN comme couche de rétention pour leur communauté payante. Vous gardez la marque ; nous apportons l'infrastructure.",
    "bridge.card1.cta": "En savoir plus →",
    "bridge.card2.label": "Pour les organisations",
    "bridge.card2.title": "Apportez SOULCHAIN à votre équipe.",
    "bridge.card2.body": "Les directions L&D utilisent SOULCHAIN pour résoudre le problème de complétion des LMS. Rituel quotidien et matching entre pairs, déployés sur les personnes que vous choisissez d'inclure.",
    "bridge.card2.cta": "En savoir plus →",

    "finalCta.title": "Demandez votre communauté SOULCHAIN.",
    "finalCta.subtitle": "Remplissez le formulaire. Nous examinons chaque demande et vous répondons sous 48 heures.",
    "finalCta.cta.primary": "Demander une communauté",
    "finalCta.cta.secondary": "Parler à notre équipe",

    "communityRequest.title": "Demander une communauté",
    "communityRequest.subhead": "Soumettre ce formulaire envoie uniquement une demande d'examen. Cela ne crée pas automatiquement une communauté.",
    "communityRequest.submit": "Envoyer la demande",
    "communityRequest.submitting": "Envoi en cours…",
    "communityRequest.confirm.title": "Demande envoyée avec succès.",
    "communityRequest.confirm.body": "Votre demande a été soumise avec succès. Notre équipe l'examinera et vous recontactera.",
    "communityRequest.fields.who": "Qui êtes-vous ?",
    "communityRequest.fields.orgName": "Nom de l'organisation",
    "communityRequest.fields.brandName": "Votre nom ou marque",
    "communityRequest.fields.name": "Nom",
    "communityRequest.fields.communityName": "Nom de communauté souhaité (optionnel)",
    "communityRequest.fields.description": "Description",
    "communityRequest.fields.descriptionPlaceholder": "Parlez-nous de votre communauté et pourquoi vous souhaitez la créer.",
    "communityRequest.fields.socialHandle": "Compte social (optionnel)",
    "communityRequest.fields.socialHandlePlaceholder": "@username ou URL LinkedIn / Instagram",
    "communityRequest.type.organization": "Organisation",
    "communityRequest.type.influencer": "Influenceur",
    "communityRequest.type.other": "Autre",
    "communityRequest.errors.nameLength": "Le nom doit contenir entre 1 et 200 caractères.",
    "communityRequest.errors.authRequired": "Connectez-vous pour envoyer une demande.",
    "communityRequest.errors.signInCta": "S'inscrire / se connecter →",
    "communityRequest.errors.submitFailed": "Une erreur s'est produite. Veuillez réessayer.",
    "communityRequest.auth.title": "Connectez-vous pour continuer",
    "communityRequest.auth.subhead": "Un compte SOULCHAIN est requis pour envoyer une demande de communauté.",
    "communityRequest.auth.tabs.signIn": "Se connecter",
    "communityRequest.auth.tabs.signUp": "Créer un compte",
    "communityRequest.auth.fields.email": "Email",
    "communityRequest.auth.fields.password": "Mot de passe",
    "communityRequest.auth.showPassword": "Afficher le mot de passe",
    "communityRequest.auth.hidePassword": "Masquer le mot de passe",
    "communityRequest.auth.fields.firstName": "Prénom",
    "communityRequest.auth.fields.lastName": "Nom",
    "communityRequest.auth.fields.otp": "Code de vérification",
    "communityRequest.auth.signIn": "Se connecter",
    "communityRequest.auth.signingIn": "Connexion…",
    "communityRequest.auth.createAccount": "Créer un compte",
    "communityRequest.auth.creating": "Création…",
    "communityRequest.auth.verify": "Vérifier le code",
    "communityRequest.auth.verifying": "Vérification…",
    "communityRequest.auth.resend": "Renvoyer le code",
    "communityRequest.auth.backToSignIn": "Retour à la connexion",
    "communityRequest.auth.signedIn": "Connecté — vous pouvez envoyer votre demande.",
    "communityRequest.auth.signOut": "Se déconnecter",
    "communityRequest.auth.otpHint": "Vérifiez votre email, puis saisissez le code reçu.",
    "communityRequest.auth.otpSent": "Un code a été envoyé à {email}.",
    "communityRequest.auth.otpResent": "Un nouveau code a été envoyé.",
    "communityRequest.auth.verifiedSignIn": "Email vérifié. Connectez-vous pour continuer.",
    "communityRequest.auth.errors.credentialsRequired": "Email et mot de passe requis.",
    "communityRequest.auth.errors.signInFailed": "Impossible de se connecter. Réessayez.",
    "communityRequest.auth.errors.signUpIncomplete": "Remplissez tous les champs (mot de passe : 8 caractères min.).",
    "communityRequest.auth.errors.signUpFailed": "Impossible de créer le compte. Réessayez.",
    "communityRequest.auth.errors.otpRequired": "Email et code de vérification requis.",
    "communityRequest.auth.errors.otpFailed": "Code invalide ou expiré. Réessayez.",
    "communityRequest.auth.errors.resendFailed": "Impossible de renvoyer le code. Réessayez.",
    "communityRequest.auth.errors.noToken": "Connexion réussie, mais aucun jeton reçu. Réessayez.",
    "communityRequest.auth.errors.verifyRequired": "Vérifiez votre email avant de continuer.",

    "apply.title": "Rejoindre la beta SOULCHAIN",
    "apply.subhead": "Deux minutes. Nous répondons sous 48 heures.",
    "apply.submit": "Rejoindre la beta",
    "apply.confirm.title": "Vérifiez votre boîte mail.",
    "apply.confirm.body": "Nous vous avons envoyé une confirmation. Nous reviendrons vers vous lors de la prochaine vague.",
    "apply.confirm.close": "Fermer",
    "apply.fields.fullName": "Nom complet",
    "apply.fields.email": "Email",
    "apply.fields.company": "Nom de l'entreprise",
    "apply.fields.role": "Rôle",
    "apply.fields.revenue": "Tranche de revenu annuel",
    "apply.fields.revenuePlaceholder": "Sélectionnez une tranche",
    "apply.fields.country": "Pays",
    "apply.fields.countryPlaceholder": "Sélectionnez un pays",
    "apply.fields.hoping": "Que souhaitez-vous obtenir ?",
    "apply.fields.whatsapp": "WhatsApp (optionnel)",
    "apply.revenue.under-1m": "< 1M$",
    "apply.revenue.1-5m": "1M$ – 5M$",
    "apply.revenue.5-25m": "5M$ – 25M$",
    "apply.revenue.25-50m": "25M$ – 50M$",
    "apply.revenue.over-50m": "> 50M$",
    "apply.country.ma": "Maroc",
    "apply.country.fr": "France",
    "apply.country.us": "États-Unis",
    "apply.country.gb": "Royaume-Uni",
    "apply.country.ae": "Émirats arabes unis",
    "apply.country.ca": "Canada",
    "apply.country.de": "Allemagne",
    "apply.country.es": "Espagne",
    "apply.country.be": "Belgique",
    "apply.country.ch": "Suisse",
    "apply.country.sn": "Sénégal",
    "apply.country.ci": "Côte d'Ivoire",
    "apply.country.other": "Autre",

    "partner.title": "Devenir partenaire community builder",
    "partner.subhead": "Parlez-nous de votre communaute. Nous reviendrons vers vous sous 5 jours.",
    "partner.submit": "Envoyer",
    "partner.confirm.title": "Demande recue.",
    "partner.confirm.body": "Merci. Nous reviendrons vers vous sous 5 jours.",
    "partner.confirm.close": "Fermer",
    "partner.fields.fullName": "Nom complet",
    "partner.fields.email": "Email",
    "partner.fields.brand": "Marque personnelle / nom de la communaute",
    "partner.fields.size": "Taille de la communaute",
    "partner.fields.sizePlaceholder": "Selectionnez une taille",
    "partner.fields.audience": "Description de l'audience",
    "partner.fields.workUrl": "Ou pouvons-nous voir votre travail ?",
    "partner.size.500-1000": "500 - 1000 membres",
    "partner.size.1000-5000": "1000 - 5000 membres",
    "partner.size.5000+": "5000+ membres",

    "form.required": "Ce champ est obligatoire.",
    "form.emailInvalid": "Veuillez entrer un email valide.",

    "builders.hero.eyebrow": "POUR LES COMMUNITY BUILDERS",
    "builders.hero.title": "Votre communauté mérite mieux qu'un flux de contenu de plus.",
    "builders.hero.subtitle": "SOULCHAIN est la couche de rétention pour les fondateurs, figures publiques et créateurs éducatifs dont l'audience paye pour apprendre d'eux. Vous apportez la marque et l'audience. Nous apportons le stack de rétention.",
    "builders.hero.cta.primary": "Postuler pour rejoindre",
    "builders.hero.cta.secondary": "Comment fonctionne le partenariat",

    "builders.ceiling.eyebrow": "LÀ OÙ ÇA DEVIENT DIFFICILE",
    "builders.ceiling.title": "Vers 5 000$ MRR. Peut-être 10 000$ sur un bon mois.",
    "builders.ceiling.imageAlt": "Réunion professionnelle autour d'une présentation sur l'apprentissage et le développement",
    "builders.ceiling.body": "Vous avez construit une audience. Vous avez lancé une communauté payante. La première vague était facile — vos vrais fans étaient là. Puis le churn a commencé. Les membres ont arrêté de poster. Les nouveaux décrochaient après le mois deux. Vous vous êtes retrouvé à produire toujours plus de contenu pour les garder engagés, et à vous épuiser à le faire.\n\nVous avez regardé Skool, Circle, Mighty Networks. Ils vous donnent un endroit où mettre les gens. Ils ne donnent pas aux gens une raison de rester.\n\nLe plafond restait le même chiffre. Et le plus dur : vous vous êtes lancé là-dedans pour enseigner, pour partager ce que vous savez. Pas pour devenir un service-client et une machine à contenu.",

    "builders.bring.eyebrow": "CE QUE NOUS APPORTONS",
    "builders.bring.title": "D'un flux de contenu à un système d'apprentissage.",
    "builders.bring.imageAlt": "Deux écrans de l'application SOULCHAIN : Brain Booster et Communautés",
    "builders.bring.card1.title": "Des insights quotidiens pour vos membres, vos thèmes",
    "builders.bring.card1.body": "Vos membres reçoivent un Brain Booster de 5 minutes chaque jour, calibré sur les thèmes que vous définissez en tant que community owner. Les thèmes sont les vôtres — votre secteur, votre méthode, votre point de vue.",
    "builders.bring.card2.title": "Un système entre pairs qui scale le 1-à-1",
    "builders.bring.card2.body": "Conversation quotidienne entre vos membres. Matching cognitif qui les associe sur leur façon de penser. Le type de relations qui exigeait votre temps personnel — désormais, dans toute votre communauté, chaque jour.",
    "builders.bring.card3.title": "Une feature Community que vous possédez vraiment",
    "builders.bring.card3.body": "SOULCHAIN vous donne la fonctionnalité Community — un espace privé déployé sous votre marque, où vos membres reçoivent un Brain Booster quotidien que vous avez choisi, en discutent ensemble et se connectent un-à-un grâce au même matching cognitif qui anime le reste de SOULCHAIN. Vous contrôlez le booster, la fréquence, la voix. Nous fournissons l'infrastructure.",

    "builders.partnership.eyebrow": "COMMENT LE PARTENARIAT FONCTIONNE",
    "builders.partnership.title": "Un partenariat simple en trois étapes.",
    "builders.partnership.step1.title": "Parler à notre équipe",
    "builders.partnership.step1.body": "Tout commence par une conversation. Nous cherchons des community builders dont l'audience valorise vraiment l'apprentissage et dont la pratique s'aligne avec l'approche SOULCHAIN. La conversation est honnête des deux côtés — nous vous disons d'emblée si le partenariat a du sens.",
    "builders.partnership.step2.title": "Co-brander",
    "builders.partnership.step2.body": "Votre espace communautaire est déployé sous votre identité, gratuitement tant que le partenariat est actif. Votre marque, vos thèmes, votre voix. Nous sommes invisibles pour vos membres.",
    "builders.partnership.step3.title": "Cumuler ensemble",
    "builders.partnership.step3.body": "Vos membres reçoivent l'expérience SOULCHAIN complète. Vous gagnez la rétention. Nous partageons le succès du partenariat.",

    "builders.fit.eyebrow": "EST-CE FAIT POUR VOUS ?",
    "builders.fit.title": "Honnêtes sur à qui cela convient.",
    "builders.fit.imageAlt": "Professionnel travaillant sur un ordinateur portable dans un espace moderne",
    "builders.fit.works.title": "Ceci fonctionne pour :",
    "builders.fit.works.item1": "Fondateurs et CEOs avec une marque personnelle basée sur le contenu",
    "builders.fit.works.item2": "Figures publiques avec une communauté payante au-delà de 500 membres",
    "builders.fit.works.item3": "Coachs et consultants dont l'audience paye pour apprendre d'eux",
    "builders.fit.works.item4": "Éducateurs avec une pratique d'enseignement sérieuse (pas juste un cours)",
    "builders.fit.doesnt.title": "Ceci ne fonctionne pas pour :",
    "builders.fit.doesnt.item1": "Des audiences qui payent pour du divertissement, pas pour de la transformation",
    "builders.fit.doesnt.item2": "Des communautés de moins de 500 membres payants (il faut de l'échelle pour justifier le partenariat)",
    "builders.fit.doesnt.item3": "Des fondateurs cherchant un produit CRM ou marketing automation",
    "builders.fit.doesnt.item4": "Des audiences dans des secteurs où nous ne pouvons pas calibrer notre matching (nous vous le dirons d'emblée)",

    "builders.finalCta.title": "Voyons s'il y a un fit.",
    "builders.finalCta.subtitle": "Une conversation de 30 minutes. Pas de slide. Pas de pitch. Nous écoutons, puis nous vous disons honnêtement si le partenariat a du sens.",
    "builders.finalCta.cta.primary": "Parler à notre équipe",

    "enterprise.hero.eyebrow": "POUR LES ORGANISATIONS",
    "enterprise.hero.title": "Infrastructure cognitive pour toute votre organisation.",
    "enterprise.hero.subtitle": "Apportez SOULCHAIN à qui vous choisissez d'inclure — du C-level aux opérationnels, d'un département à toute l'entreprise. Des insights quotidiens qu'ils utilisent vraiment. Un réseau de pairs auquel ils font vraiment confiance.",
    "enterprise.hero.cta.primary": "Postuler pour rejoindre",
    "enterprise.hero.cta.secondary": "Voir ce que chaque membre reçoit",

    "enterprise.completion.eyebrow": "LÀ OÙ ÇA DEVIENT DIFFICILE",
    "enterprise.completion.title": "Votre LMS est plein. Pas vos taux d'achèvement.",
    "enterprise.completion.imageAlt": "Réunion professionnelle autour d'une table de conférence",
    "enterprise.completion.body": "Vous avez licencié les meilleures bibliothèques de contenu, déployé les bons outils, fait passer des pulse surveys. Toujours la même poignée d'employés qui engage. Tous les autres disparaissent après l'onboarding. Votre CEO demande le ROI. Votre CHRO demande la culture. Votre CFO demande pourquoi cette ligne budgétaire grossit. Vous savez que l'apprentissage compte. Vous ne pouvez pas toujours prouver qu'il se produit.\n\nLes programmes en cohorte fonctionnent — quand vous pouvez les financer. Les cours en autonomie scalent — quand quelqu'un les termine. Vous n'avez pas besoin d'une bibliothèque de plus. Vous avez besoin de quelque chose que les gens utilisent vraiment.",

    "enterprise.bring.eyebrow": "CE QUE SOULCHAIN APPORTE À VOTRE ORGANISATION",
    "enterprise.bring.title": "Une infrastructure cognitive. Pas un catalogue de cours de plus.",
    "enterprise.bring.imageAlt": "Deux écrans de l'application SOULCHAIN : Brain Booster et Communautés",
    "enterprise.bring.card1.title": "De la profondeur quotidienne, à l'échelle que vous choisissez",
    "enterprise.bring.card1.subtitle": "Cinq minutes de pensée structurée par personne, par jour.",
    "enterprise.bring.card1.body": "SOULCHAIN délivre un Brain Booster quotidien sur les thèmes que vous définissez — stratégie, culture IA, régulation, comportement client, leadership, ce qui compte pour votre business. Une pratique quotidienne qui s'accumule, pas un catalogue à mettre en favori.",
    "enterprise.bring.card2.title": "Vous décidez qui, vous décidez quoi",
    "enterprise.bring.card2.subtitle": "Votre équipe. Vos thèmes. Votre choix.",
    "enterprise.bring.card2.body": "Incluez uniquement les dirigeants, un département, ou chaque personne de l'organisation. Les thèmes sont les vôtres à définir et à ajuster par trimestre, par priorité, selon ce que votre business affronte. Chaque membre garde aussi ses thèmes personnels.",
    "enterprise.bring.card3.title": "La rétention talent cachée dans l'apprentissage",
    "enterprise.bring.card3.subtitle": "Un réseau qui fait grandir vos équipes sans les perdre.",
    "enterprise.bring.card3.body": "Les membres échangent avec des pairs au-delà des fonctions et au-delà des entreprises. Ils grandissent sans devoir partir. Le réseau, c'est la rétention talent cachée dans l'apprentissage.",

    "enterprise.sticks.eyebrow": "POURQUOI ÇA TIENT",
    "enterprise.sticks.title": "La plupart des programmes d'apprentissage sont oubliés avant de s'ancrer. Ce n'en est pas un.",
    "enterprise.sticks.stat1": "64–90%",
    "enterprise.sticks.stat1.label": "Taux d'achèvement avec apprentissage communautaire et en cohorte",
    "enterprise.sticks.stat2": "3–15%",
    "enterprise.sticks.stat2.label": "Taux d'achèvement avec l'apprentissage classique en autonomie",
    "enterprise.sticks.attribution": "Référence sectorielle — recherche sur l'apprentissage en cohorte, 2024.",
    "enterprise.sticks.body": "SOULCHAIN est construit sur le stack de rétention — micro-stimulation quotidienne, redevabilité entre pairs, conversation appliquée. C'est pour ça que ce que votre équipe apprend le premier jour se retrouve encore dans ses décisions au quatre-vingt-dixième.",

    "enterprise.member.eyebrow": "CE QUE CHAQUE MEMBRE REÇOIT",
    "enterprise.member.title": "Le même produit. Configuré pour votre équipe.",
    "enterprise.member.card1.title": "Brain Booster personnalisé",
    "enterprise.member.card1.body": "Un insight de cinq minutes par jour, en texte, audio ou podcast. Le booster de chaque membre est calibré sur les thèmes que votre organisation définit et sur ses thèmes personnels. Un quiz court à la fin de chaque booster vérifie que l'insight s'est ancré.",
    "enterprise.member.card2.title": "Daily Groups",
    "enterprise.member.card2.body": "Les membres sont regroupés chaque jour avec des pairs au sein de l'organisation dont le booster recoupe le leur. Des conversations transversales qui n'auraient pas lieu organiquement — révélées par ce que les gens pensent réellement.",
    "enterprise.member.card3.title": "Peers",
    "enterprise.member.card3.body": "Chaque membre voit une liste de collègues avec qui il est cognitivement compatible — des personnes qu'il peut contacter directement quand il a besoin d'un regard extérieur. Des connexions construites par affinité cognitive, pas par l'organigramme.",
    "enterprise.member.card4.title": "Near By",
    "enterprise.member.card4.body": "Pour les organisations à équipes distribuées, Near By fait remonter les membres de votre communauté qui sont physiquement proches — transformant les connexions entre pairs en présence physique chaque fois que possible.",

    "enterprise.how.eyebrow": "COMMENT ÇA FONCTIONNE POUR VOTRE ÉQUIPE",
    "enterprise.how.title": "Trois étapes, puis ça tourne.",
    "enterprise.how.step1.caption": "Embarquement",
    "enterprise.how.step1.title": "Embarquez vos équipes",
    "enterprise.how.step1.body": "Nous vous aidons à intégrer vos équipes sur SOULCHAIN — les rôles et personnes que vous choisissez, à la portée que vous voulez. Chaque membre crée son profil et choisit ses thèmes de départ.",
    "enterprise.how.step2.caption": "Thématiques",
    "enterprise.how.step2.title": "Choisissez ce qui compte",
    "enterprise.how.step2.body": "Vous définissez les thèmes qui correspondent à vos priorités — culture IA, évolutions réglementaires, dynamiques de marché, modèles de leadership, sujets sectoriels. Chaque membre garde aussi ses thèmes personnels.",
    "enterprise.how.step3.caption": "Effet composé",
    "enterprise.how.step3.title": "Laissez l'effet composé opérer",
    "enterprise.how.step3.body": "Brain Boosters quotidiens, échanges entre pairs, réflexion appliquée. L'effet composé se voit dans la préparation des boards, les décisions de recrutement et les arbitrages que vos équipes prennent sous pression.",

    "enterprise.trust.eyebrow": "PARTENAIRES ENTREPRISE FONDATEURS",
    "enterprise.trust.body": "Nous ouvrons SOULCHAIN à une petite cohorte de partenaires entreprise fondateurs. Si vous voulez que votre équipe en fasse partie, la conversation commence par un appel.",
    "enterprise.trust.cta": "Démarrer la conversation",

    "enterprise.finalCta.title": "La conversation vous coûte trente minutes.",
    "enterprise.finalCta.subtitle": "Nous ouvrons SOULCHAIN à une petite cohorte de partenaires entreprise fondateurs. Si vous voulez que votre équipe en fasse partie, la conversation commence par un appel au cours duquel nous vous montrons l'expérience membre, nous parcourons les thèmes qui correspondent à votre équipe, et nous voyons s'il y a un match — pas de slide, pas de script.",
    "enterprise.finalCta.cta": "Réserver un appel de découverte"
  },
  en: {
    "nav.founders": "For Founders",
    "nav.home": "Home",
    "nav.about": "About",
    "nav.useCases": "Use cases",
    "nav.communityBuilders": "For Community Builders",
    "nav.organisations": "For Organisations",
    "nav.cta.apply": "Apply",
    "nav.cta.beta": "Request a community",
    "nav.cta.partner": "Become a partner",
    "nav.cta.bookCall": "Book a call",
    "nav.cta.discovery": "Book a discovery call",

    "home.hero.eyebrow": "PEER LEARNING FOR PEOPLE WHO REFUSE TO FALL BEHIND",
    "home.hero.title.lead": "Be curious. Have purpose. Stay ahead.",
    "home.hero.title.accent": "Together.",
    "home.hero.subtitle": "SOULCHAIN pairs a daily insight tuned to where you are with a peer network matched on how you think. Built around what AI cannot give you: human connection, lived experience, and the people who've been where you're going.",
    "home.hero.cta.primary": "Apply to join",
    "home.hero.cta.secondary": "See how it works",

    "home.doors.eyebrow": "THREE DOORS · ONE PLATFORM",
    "home.doors.title": "One product. Three ways in.",
    "home.doors.subtitle": "We've built SOULCHAIN around three audiences. The product is one. The doors are three.",
    "home.doors.founders.eyebrow": "FOR FOUNDERS",
    "home.doors.founders.title": "Built for founders, first.",
    "home.doors.founders.body": "A daily habit and a cohort of operators learning alongside you. For founders running real companies who refuse to learn alone.",
    "home.doors.founders.link": "Learn more →",
    "home.doors.builders.eyebrow": "FOR COMMUNITY BUILDERS",
    "home.doors.builders.title": "For audiences that pay to learn from you.",
    "home.doors.builders.body": "The retention layer for founders, public figures, and education-driven creators. You bring the brand and the audience. We bring the system that makes them stay.",
    "home.doors.builders.link": "Learn more →",
    "home.doors.enterprise.eyebrow": "FOR ORGANISATIONS",
    "home.doors.enterprise.title": "For the teams behind the work.",
    "home.doors.enterprise.body": "Cognitive infrastructure for the people you choose to include. From execs to operators. From one department to the whole company.",
    "home.doors.enterprise.link": "Learn more →",

    "home.pillars.eyebrow": "WHAT SOULCHAIN ACTUALLY IS",
    "home.pillars.title": "Built around what AI cannot replicate.",
    "home.pillars.subtitle": "SOULCHAIN is not a learning app. It's a peer-learning platform organised around four pillars.",
    "home.pillars.p1.title": "Daily habit",
    "home.pillars.p1.body": "Five minutes a day. A personalised insight tuned to where you are, in the format that fits your moment: text, audio, or podcast. The rhythm AI subscriptions don't structure.",
    "home.pillars.p1.bodyLine1": "Five minutes a day. A personalised insight tuned to where you are,",
    "home.pillars.p1.bodyLine2": "in the format that fits your moment: text, audio, or podcast.",
    "home.pillars.p1.bodyLine3": "The rhythm AI subscriptions don't structure.",
    "home.pillars.p2.title": "Cognitive matching",
    "home.pillars.p2.body": "An algorithm that pairs you with people who think the way you do. The more you use SOULCHAIN, the better the matches get. This is the part competitors cannot replicate by adding features.",
    "home.pillars.p3.title": "Peer presence",
    "home.pillars.p3.body": "Real people in real conversations. Operators who've made the call you're facing this week. What ChatGPT, Claude, and Perplexity fundamentally cannot replace, no matter how good the model gets.",
    "home.pillars.p4.title": "Lived experience",
    "home.pillars.p4.body": "A community is more than a contact list. SOULCHAIN gives you the structure to turn insight into conversation, conversation into relationship, and relationship into the kind of thinking you can't do alone.",

    "home.whynow.eyebrow": "Why now",
    "home.whynow.title": "Three conditions just opened the window.",
    "home.whynow.col1.stat": "$20",
    "home.whynow.col1.caption": "Per month",
    "home.whynow.col1.title": "AI absorbs the substitutable.",
    "home.whynow.col1.body": "Research, summarisation, content delivery — commoditised by generative AI at $20 a month. What remains valuable is what AI cannot replicate: peer presence, cognitive matching, habit.",
    "home.whynow.col2.stat": "55%",
    "home.whynow.col2.caption": "Of CEOs · 2024",
    "home.whynow.col2.title": "Isolation is now publicly named.",
    "home.whynow.col2.body": "55% of CEOs reported mental health issues in 2024, up 24 points from 2023. Public statements from leaders at Airbnb, Apple, PepsiCo. The cost of going alone has become measurable.",
    "home.whynow.col3.stat": "#1",
    "home.whynow.col3.caption": "Reason cited",
    "home.whynow.col3.title": "Stage mismatch is the universal complaint.",
    "home.whynow.col3.body": "The number-one reason people leave traditional peer groups is being grouped with peers who don't match their stage. Existing models can't solve this at scale. Cognitive matching can.",

    "home.proof.eyebrow": "Why daily habit + peer presence works",
    "home.proof.headlineLead": "Most learning is forgotten before it lands. ",
    "home.proof.headlineEm": "This isn't learning.",
    "home.proof.stat1.range": "64–90",
    "home.proof.stat1.label": "Completion rate with community + cohort learning",
    "home.proof.stat2.range": "3–15",
    "home.proof.stat2.label": "Completion rate with traditional self-paced learning",
    "home.proof.citation": "Industry benchmark — cohort-based learning research, 2024.",
    "home.proof.bodyLead": "SOULCHAIN is built on the retention stack: ",
    "home.proof.bodyStrong": "daily micro-stimulation, peer accountability, applied conversation.",
    "home.proof.bodyTail": " What you engage with on day one shows up in your decisions on day ninety.",

    "home.quote.text": "Be curious. Have a purpose. Give meaning to your life. Cultivate the meaning. Love that life.",
    "home.quote.attribution": "KARIM AMOR · FOUNDER, EPINEON",

    "home.about.eyebrow": "ABOUT EPINEON",
    "home.about.title": "A vertical of Epineon.",
    "home.about.body": "SOULCHAIN is built by Epineon, a venture working on what we think AI cannot replicate: cognitive matching, daily habit, peer presence, identity. SOULCHAIN is the first application of that thesis. More to come.",
    "home.about.link": "Visit Epineon →",

    "home.cta.title": "Request your SOULCHAIN community.",
    "home.cta.subtitle": "Submit a request. We review every application manually and get back to you quickly.",
    "home.cta.primary": "Request a community",
    "home.cta.builders": "Building a community? Talk to us →",
    "home.cta.enterprise": "Bringing it to your team? Book a call →",

    "footer.tagline": "Epineon's initiative to steward organization for their resilience",
    "footer.product.founders": "For Founders",
    "footer.product.communityBuilders": "For Community Builders",
    "footer.product.organisations": "For Organisations",
    "footer.downloadApp.title": "Download our app",
    "footer.company.about": "About Epineon",
    "footer.company.press": "Press",
    "footer.company.careers": "Careers",
    "footer.legal.terms": "Terms",
    "footer.legal.privacy": "Privacy",
    "footer.legal.cookies": "Cookies",
    "footer.legal.contact": "Contact",
    "footer.copyright": "© 2026 Epineon · SOULCHAIN. Casablanca · Built for decision-makers worldwide.",

    "hero.eyebrow": "FOR FOUNDERS",
    "hero.title.lead": "AI gives you answers.",
    "hero.title.accent": "It doesn't give you conviction.",
    "hero.subtitle": "Five-minute daily insights tuned to where you are. Daily peer conversations with operators at your level. AI cognitive matching that builds the circle you didn't think you could rebuild.",
    "hero.cta.primary": "Apply to join",
    "hero.cta.secondary": "How it works",
    "hero.brandLine": "A vertical of Epineon",

    "trust.label": "Founders building $1M to $50M companies. France · USA · MENA · Asia.",
    "trust.stat1": "4 continents.",
    "trust.stat2": "Operators only.",

    "pain.eyebrow": "WHERE IT GETS HARD",
    "pain.title": "At this level, learning isn't a content problem. It's a peer problem.",
    "pain.body": "You read constantly. You listen to podcasts at 1.5x. You ask ChatGPT thirty questions a day. And you still feel the gap. You don't have enough people who operate at your scale, who'll challenge your thinking honestly, who've been in the room when the decision you're facing this week was actually made. The more you grow, the smaller that circle gets.",
    "pain.imageAlt": "Executive working alone at night in a boardroom overlooking the city",

    "bring.eyebrow": "WHAT WE BRING",
    "bring.title": "A learning system built around what AI cannot replicate.",
    "bring.card1.title": "Personalised Brain Booster",
    "bring.card1.body": "Five minutes a day, in the format that fits your moment. Text, audio, or podcast. Tuned to your sector, your stage, your themes. The themes you start with (Pro, Health, Sport, Family, Hobbies) are suggestions; you add your own. Each booster ends with a quick quiz that verifies the insight actually landed.",
    "bring.card2.title": "Near By",
    "bring.card2.body": "Wherever you are, SOULCHAIN surfaces members of your community who are nearby — people you've already connected with, now within reach. Send a quick message. Organise a coffee. The platform turns peer matching into real-world presence.",
    "bring.card3.title": "Daily Groups",
    "bring.card3.body": "Each day, SOULCHAIN groups you with other members whose booster overlaps with yours. One topic. One conversation. When the day ends, the discussion moves on. No dead chats. No infinite scroll.",
    "bring.card4.title": "Peers",
    "bring.card4.body": "A list of people in your community who think the way you do. Connect with anyone on the list directly. The kind of one-to-one relationships you'd take a flight for — surfaced by an algorithm trained on cognitive affinity.",

    "how.eyebrow": "HOW IT WORKS",
    "how.title": "Four steps. Then it runs.",
    "how.step1.title": "Join the beta",
    "how.step1.body": "A two-minute form. We add new members as we scale. Once you're in, you set up your profile and pick your starting themes.",
    "how.step2.title": "Get matched",
    "how.step2.body": "The cognitive matching algorithm pairs you with daily peer conversations and one-to-one peer connections based on how you think, your stage, your sector.",
    "how.step3.title": "Show up daily",
    "how.step3.body": "Five minutes of Brain Booster in the morning. A peer conversation when you have ten. A list of compatible peers when you have a question that needs a human answer.",
    "how.step4.title": "Compound",
    "how.step4.body": "Daily habit, peer accountability, applied conversation. Three months in, the sharpening shows up in the decisions you're making under pressure.",
    "how.outcome": "Better decisions. Stronger network. Less isolation.",

    "whyFounders.eyebrow": "WHY FOUNDERS FIRST",
    "whyFounders.title": "A focus, not a filter.",
    "whyFounders.highlight": "Karim's twenty years of operator experience",
    "whyFounders.body": "SOULCHAIN is built for anyone who refuses to learn alone. But we built the first version for founders because that's where our network is sharpest — where Karim's twenty years of operator experience and the people in his circle line up. The founder lens shapes the messaging, the tone, and the early cohort. The product itself works for whoever joins.\n\nIf you're a founder, the door is direct. If you're not, the door is the same — the conversations just bring a different angle.",

    "whynow.eyebrow": "WHY NOW",
    "whynow.title": "Three conditions just opened the window.",
    "whynow.col1.stat": "$20",
    "whynow.col1.title": "AI absorbs the substitutable.",
    "whynow.col1.body": "Research, summarisation, content delivery - commoditised by generative AI at $20 a month. What remains valuable is what AI cannot replicate: peer presence, cognitive matching, ritual.",
    "whynow.col2.stat": "55%",
    "whynow.col2.title": "Isolation is now publicly named.",
    "whynow.col2.body": "55% of CEOs reported mental health issues in 2024 - up 24 points from 2023. Statements from leaders at Airbnb, Apple, PepsiCo. The cost of going alone has become measurable.",
    "whynow.col3.stat": "#1",
    "whynow.col3.title": "Stage mismatch is the universal complaint.",
    "whynow.col3.body": "The number-one reason CEOs leave Vistage, YPO, and EO is being grouped with peers who don't match their stage. Existing models can't solve this at scale. Cognitive matching can.",

    "cohort.eyebrow": "THE FOUNDERS' COHORT",
    "cohort.title": "Chosen, not contacted.",
    "cohort.body": "SOULCHAIN is a peer network with a door. We screen every application. We're looking for founders and CEOs who run companies between $1M and $50M in revenue, who've outgrown their first network, and who are willing to show up for the daily ritual. The selection is what makes the room worth being in. The cohort is what the platform is for.",
    "cohort.cta": "Apply to join the cohort",

    "bridge.eyebrow": "NOT A FOUNDER?",
    "bridge.card1.label": "For community builders",
    "bridge.card1.title": "Bring SOULCHAIN to your audience.",
    "bridge.card1.body": "Founders, public figures, and education-driven creators use SOULCHAIN as the retention layer for their paid community. We give you the infrastructure; you keep the brand.",
    "bridge.card1.cta": "Learn more →",
    "bridge.card2.label": "For organisations",
    "bridge.card2.title": "Bring SOULCHAIN to your team.",
    "bridge.card2.body": "L&D leaders use SOULCHAIN to solve the LMS completion problem. Daily ritual + peer matching, deployed across the people you choose to include.",
    "bridge.card2.cta": "Learn more →",

    "finalCta.title": "Request your SOULCHAIN community.",
    "finalCta.subtitle": "Fill out the form. We review every request and respond within 48 hours.",
    "finalCta.cta.primary": "Request a community",
    "finalCta.cta.secondary": "Talk to our team",

    "communityRequest.title": "Request a Community",
    "communityRequest.subhead": "Submitting this form only sends a request for review. It does not automatically create a community.",
    "communityRequest.submit": "Submit Request",
    "communityRequest.submitting": "Submitting…",
    "communityRequest.confirm.title": "Request submitted successfully.",
    "communityRequest.confirm.body": "Your request has been submitted successfully. Our team will review it and get back to you.",
    "communityRequest.fields.who": "Who are you?",
    "communityRequest.fields.orgName": "Organization Name",
    "communityRequest.fields.brandName": "Your Name or Brand",
    "communityRequest.fields.name": "Name",
    "communityRequest.fields.communityName": "Desired Community Name (optional)",
    "communityRequest.fields.description": "Description",
    "communityRequest.fields.descriptionPlaceholder": "Tell us about your community and why you'd like to create it.",
    "communityRequest.fields.socialHandle": "Social Handle (optional)",
    "communityRequest.fields.socialHandlePlaceholder": "@username or a LinkedIn / Instagram URL",
    "communityRequest.type.organization": "Organization",
    "communityRequest.type.influencer": "Influencer",
    "communityRequest.type.other": "Other",
    "communityRequest.errors.nameLength": "Name must be between 1 and 200 characters.",
    "communityRequest.errors.authRequired": "Sign in to submit a request.",
    "communityRequest.errors.signInCta": "Sign up / sign in →",
    "communityRequest.errors.submitFailed": "Something went wrong. Please try again.",
    "communityRequest.auth.title": "Sign in to continue",
    "communityRequest.auth.subhead": "A SOULCHAIN account is required to submit a community request.",
    "communityRequest.auth.tabs.signIn": "Sign in",
    "communityRequest.auth.tabs.signUp": "Create account",
    "communityRequest.auth.fields.email": "Email",
    "communityRequest.auth.fields.password": "Password",
    "communityRequest.auth.showPassword": "Show password",
    "communityRequest.auth.hidePassword": "Hide password",
    "communityRequest.auth.fields.firstName": "First name",
    "communityRequest.auth.fields.lastName": "Last name",
    "communityRequest.auth.fields.otp": "Verification code",
    "communityRequest.auth.signIn": "Sign in",
    "communityRequest.auth.signingIn": "Signing in…",
    "communityRequest.auth.createAccount": "Create account",
    "communityRequest.auth.creating": "Creating…",
    "communityRequest.auth.verify": "Verify code",
    "communityRequest.auth.verifying": "Verifying…",
    "communityRequest.auth.resend": "Resend code",
    "communityRequest.auth.backToSignIn": "Back to sign in",
    "communityRequest.auth.signedIn": "Signed in — you can submit your request.",
    "communityRequest.auth.signOut": "Sign out",
    "communityRequest.auth.otpHint": "Check your email, then enter the code you received.",
    "communityRequest.auth.otpSent": "A code was sent to {email}.",
    "communityRequest.auth.otpResent": "A new code was sent.",
    "communityRequest.auth.verifiedSignIn": "Email verified. Sign in to continue.",
    "communityRequest.auth.errors.credentialsRequired": "Email and password are required.",
    "communityRequest.auth.errors.signInFailed": "Unable to sign in. Please try again.",
    "communityRequest.auth.errors.signUpIncomplete": "Fill in all fields (password: 8+ characters).",
    "communityRequest.auth.errors.signUpFailed": "Unable to create account. Please try again.",
    "communityRequest.auth.errors.otpRequired": "Email and verification code are required.",
    "communityRequest.auth.errors.otpFailed": "Invalid or expired code. Please try again.",
    "communityRequest.auth.errors.resendFailed": "Unable to resend code. Please try again.",
    "communityRequest.auth.errors.noToken": "Signed in, but no token was returned. Please try again.",
    "communityRequest.auth.errors.verifyRequired": "Verify your email before continuing.",

    "apply.title": "Join the SOULCHAIN beta",
    "apply.subhead": "Two minutes. We'll respond within 48 hours.",
    "apply.submit": "Join the beta",
    "apply.confirm.title": "Check your inbox.",
    "apply.confirm.body": "We've sent you a confirmation. We'll be in touch as we add the next wave of members.",
    "apply.confirm.close": "Close",
    "apply.fields.fullName": "Full name",
    "apply.fields.email": "Email",
    "apply.fields.company": "Company name",
    "apply.fields.role": "Role",
    "apply.fields.revenue": "Annual revenue range",
    "apply.fields.revenuePlaceholder": "Select a range",
    "apply.fields.country": "Country",
    "apply.fields.countryPlaceholder": "Select a country",
    "apply.fields.hoping": "What are you hoping to get?",
    "apply.fields.whatsapp": "WhatsApp (optional)",
    "apply.revenue.under-1m": "< $1M",
    "apply.revenue.1-5m": "$1M–$5M",
    "apply.revenue.5-25m": "$5M–$25M",
    "apply.revenue.25-50m": "$25M–$50M",
    "apply.revenue.over-50m": "> $50M",
    "apply.country.ma": "Morocco",
    "apply.country.fr": "France",
    "apply.country.us": "United States",
    "apply.country.gb": "United Kingdom",
    "apply.country.ae": "United Arab Emirates",
    "apply.country.ca": "Canada",
    "apply.country.de": "Germany",
    "apply.country.es": "Spain",
    "apply.country.be": "Belgium",
    "apply.country.ch": "Switzerland",
    "apply.country.sn": "Senegal",
    "apply.country.ci": "Côte d'Ivoire",
    "apply.country.other": "Other",

    "partner.title": "Become a community builder partner",
    "partner.subhead": "Tell us about your community. We'll respond within 5 days.",
    "partner.submit": "Submit",
    "partner.confirm.title": "Inquiry received.",
    "partner.confirm.body": "We'll respond within 5 days.",
    "partner.confirm.close": "Close",
    "partner.fields.fullName": "Full name",
    "partner.fields.email": "Email",
    "partner.fields.brand": "Personal brand / community name",
    "partner.fields.size": "Community size",
    "partner.fields.sizePlaceholder": "Select a size",
    "partner.fields.audience": "Audience description",
    "partner.fields.workUrl": "Where can we see your work?",
    "partner.size.500-1000": "500 - 1000 members",
    "partner.size.1000-5000": "1000 - 5000 members",
    "partner.size.5000+": "5000+ members",

    "form.required": "This field is required.",
    "form.emailInvalid": "Please enter a valid email address.",

    "builders.hero.eyebrow": "FOR COMMUNITY BUILDERS",
    "builders.hero.title": "Your community deserves more than another content stream.",
    "builders.hero.subtitle": "SOULCHAIN is the retention layer for founders, public figures, and education-driven creators whose audience pays to learn from them. You bring the brand and the audience. We bring the retention stack.",
    "builders.hero.cta.primary": "Apply to join",
    "builders.hero.cta.secondary": "How partnership works",

    "builders.ceiling.eyebrow": "WHERE IT GETS HARD",
    "builders.ceiling.title": "Around $5K MRR. Maybe $10K on a good month.",
    "builders.ceiling.imageAlt": "Professional meeting viewing a presentation on learning and development",
    "builders.ceiling.body": "You built an audience. You launched a paid community. The first wave was easy - your true fans showed up. Then the churn started. Members stopped posting. New ones drifted away after month two. You found yourself producing more content to keep them engaged, and burning out doing it. You looked at Skool, Circle, Mighty Networks. They give you a place to put people. They don't give people a reason to stay. The ceiling kept looking like the same number. And the harder part: you got into this to teach, to share what you know. Not to become a customer-service desk and a content machine.",

    "builders.bring.eyebrow": "WHAT WE BRING",
    "builders.bring.title": "From content stream to learning system.",
    "builders.bring.imageAlt": "Two SOULCHAIN app screens showing Brain Booster and Communities",
    "builders.bring.card1.title": "Daily insights for your members, your themes",
    "builders.bring.card1.body": "Your members receive a 5-minute Brain Booster every day, tuned to the themes you set as their community owner. The themes are yours - your sector, your method, your point of view.",
    "builders.bring.card2.title": "A peer system that scales the 1-on-1 feel",
    "builders.bring.card2.body": "Daily peer conversation among your members. Cognitive matching that pairs them on how they think. The kind of relationships that used to require your personal time - now happening across your community, every day.",
    "builders.bring.card3.title": "A Community feature you actually own",
    "builders.bring.card3.body": "SOULCHAIN gives you the Community feature — a private space deployed under your brand, where your members receive a daily Brain Booster you've curated, discuss it together, and connect one to one through the same cognitive matching that powers the rest of SOULCHAIN. You control the booster, the frequency, the voice. We provide the infrastructure.",

    "builders.partnership.eyebrow": "HOW PARTNERSHIP WORKS",
    "builders.partnership.title": "A simple three-step partnership.",
    "builders.partnership.step1.title": "Talk to our team",
    "builders.partnership.step1.body": "We start with a conversation. We're looking for community builders whose audience genuinely values learning and whose practice aligns with the SOULCHAIN approach. The conversation is honest both ways — we tell you upfront if the partnership makes sense.",
    "builders.partnership.step2.title": "Co-brand",
    "builders.partnership.step2.body": "Your community space is deployed under your identity, free as long as the partnership is active. Your branding, your themes, your voice. We're invisible to your members.",
    "builders.partnership.step3.title": "Compound together",
    "builders.partnership.step3.body": "Your members get the full SOULCHAIN experience. You get the retention. We share in the success of the partnership.",

    "builders.fit.eyebrow": "IS THIS FOR YOU?",
    "builders.fit.title": "Honest about who this fits.",
    "builders.fit.imageAlt": "Professional working on a laptop in a modern lounge setting",
    "builders.fit.works.title": "Works for:",
    "builders.fit.works.item1": "Founders and CEOs running content-driven personal brands",
    "builders.fit.works.item2": "Public figures with a paid community above 500 members",
    "builders.fit.works.item3": "Coaches and consultants whose audience pays to learn from them",
    "builders.fit.works.item4": "Educators with a serious teaching practice",
    "builders.fit.doesnt.title": "Doesn't work for:",
    "builders.fit.doesnt.item1": "Audiences that pay for entertainment, not transformation",
    "builders.fit.doesnt.item2": "Communities under 500 paying members",
    "builders.fit.doesnt.item3": "Founders looking for a CRM or marketing automation product",
    "builders.fit.doesnt.item4": "Audiences in industries we can't tune our content matching for",

    "builders.finalCta.title": "Let's see if there's a fit.",
    "builders.finalCta.subtitle": "A 30-minute conversation. No deck. No sales pitch. We'll listen, then tell you honestly if the partnership makes sense.",
    "builders.finalCta.cta.primary": "Talk to our team",

    "enterprise.hero.eyebrow": "FOR ORGANISATIONS",
    "enterprise.hero.title": "Cognitive infrastructure for your whole organisation.",
    "enterprise.hero.subtitle": "Bring SOULCHAIN to anyone you choose to include - from execs to operators, from one department to the whole company. Daily insights they actually use. A peer network they actually trust.",
    "enterprise.hero.cta.primary": "Apply to join",
    "enterprise.hero.cta.secondary": "See what each member gets",

    "enterprise.completion.eyebrow": "WHERE IT GETS HARD",
    "enterprise.completion.title": "Your LMS is full. Your completion rates are not.",
    "enterprise.completion.imageAlt": "Professional meeting around a conference table",
    "enterprise.completion.body": "You've licensed the best content libraries, deployed the right tools, run pulse surveys. The same handful of employees engage. Everyone else disappears after onboarding. Your CEO asks about ROI. Your CHRO asks about culture. Your CFO asks why the line item keeps growing. You know learning matters. You can't always prove it's happening. Cohort programmes work - when you can afford them. Self-paced courses scale - when anyone finishes them. You don't need another content library. You need something people actually use.",

    "enterprise.bring.eyebrow": "WHAT SOULCHAIN BRINGS TO YOUR ORGANISATION",
    "enterprise.bring.title": "Cognitive infrastructure. Not another course catalogue.",
    "enterprise.bring.imageAlt": "Two SOULCHAIN app screens showing Brain Booster and Communities",
    "enterprise.bring.card1.title": "Daily depth, at any scale you choose",
    "enterprise.bring.card1.subtitle": "Five minutes of structured thinking per person, per day.",
    "enterprise.bring.card1.body": "SOULCHAIN delivers a daily Brain Booster on the themes you define — strategy, AI culture, regulation, customer behaviour, leadership, whatever matters to your business. A daily practice that compounds, not a catalogue to bookmark.",
    "enterprise.bring.card2.title": "You decide who, you decide what",
    "enterprise.bring.card2.subtitle": "Your team. Your themes. Your call.",
    "enterprise.bring.card2.body": "Include only leaders, one department, or everyone in the organisation. You define and adjust themes by quarter, by priority, based on what your business is facing. Each member keeps personal themes too.",
    "enterprise.bring.card3.title": "Talent retention hiding inside learning",
    "enterprise.bring.card3.subtitle": "A network that grows your people without losing them.",
    "enterprise.bring.card3.body": "Members connect with peers across functions and across companies. They grow without having to leave. The network is talent retention hiding inside learning.",

    "enterprise.sticks.eyebrow": "WHY IT STICKS",
    "enterprise.sticks.title": "Most learning programmes are forgotten before they land. This isn't a programme.",
    "enterprise.sticks.stat1": "64-90%",
    "enterprise.sticks.stat1.label": "Completion rate with community + cohort learning",
    "enterprise.sticks.stat2": "3-15%",
    "enterprise.sticks.stat2.label": "Completion rate with traditional self-paced learning",
    "enterprise.sticks.attribution": "Industry benchmark - cohort-based learning research, 2024.",
    "enterprise.sticks.body": "SOULCHAIN is built on the retention stack - daily micro-stimulation, peer accountability, applied conversation. That's why what your team learns on day one is still showing up in their decisions on day ninety.",

    "enterprise.member.eyebrow": "WHAT EACH MEMBER RECEIVES",
    "enterprise.member.title": "The same product. Curated for your team.",
    "enterprise.member.card1.title": "Personalised Brain Booster",
    "enterprise.member.card1.body": "A five-minute insight per day, in text, audio, or podcast. Each member's booster is tuned to the themes your organisation sets and to their personal themes. A short quiz at the end of each booster verifies the insight landed.",
    "enterprise.member.card2.title": "Daily Groups",
    "enterprise.member.card2.body": "Members get grouped each day with peers across the organisation whose booster overlaps with theirs. Cross-team conversations that wouldn't happen organically — surfaced by what people are actually thinking about.",
    "enterprise.member.card3.title": "Peers",
    "enterprise.member.card3.body": "Each member sees a list of colleagues they're cognitively compatible with — people they can reach out to directly when they need a sounding board. Talent connections built by cognitive affinity, not org chart.",
    "enterprise.member.card4.title": "Near By",
    "enterprise.member.card4.body": "For organisations with distributed teams, Near By surfaces members of your community who are physically close — turning peer connections into in-person presence whenever possible.",

    "enterprise.how.eyebrow": "HOW IT WORKS FOR YOUR TEAM",
    "enterprise.how.title": "Three steps, then it runs.",
    "enterprise.how.step1.caption": "Onboarding",
    "enterprise.how.step1.title": "Onboard your people",
    "enterprise.how.step1.body": "We help you bring your people onto SOULCHAIN - the roles and individuals you choose, at the scope you want. Each member sets up their profile and picks their starting themes.",
    "enterprise.how.step2.caption": "Curation",
    "enterprise.how.step2.title": "Curate what matters",
    "enterprise.how.step2.body": "You set the themes that align with your business priorities - AI literacy, regulatory shifts, market dynamics, leadership models, sector-specific topics. Each member also keeps their personal themes.",
    "enterprise.how.step3.caption": "Compound",
    "enterprise.how.step3.title": "Let it compound",
    "enterprise.how.step3.body": "Daily Brain Boosters, peer exchange, applied reflection. The compounding shows up in board prep, hiring decisions, and the calls your people make under pressure.",

    "enterprise.trust.eyebrow": "FOUNDING ENTERPRISE PARTNERS",
    "enterprise.trust.body": "We're opening SOULCHAIN to a small cohort of founding enterprise partners. If you want your team in that cohort, the conversation starts with a call.",
    "enterprise.trust.cta": "Start the conversation",

    "enterprise.finalCta.title": "The conversation costs you thirty minutes.",
    "enterprise.finalCta.subtitle": "We're opening SOULCHAIN to a small cohort of founding enterprise partners. If you want your team in that cohort, the conversation starts with a call where we'll show you the member experience, discuss the themes that fit your team, and see if there's a match — no deck, no script.",
    "enterprise.finalCta.cta": "Book a discovery call"
  }
};
